...
const BASE_REQUIRED_COLUMNS = ['userType', 'name', 'phone', 'email', 'rfidCode'] as const;
...
function ImportExportTab() {
  ...
  const REQUIRED_COLUMNS_BY_USER_TYPE: Record<UserType, Array<(typeof EXPECTED_COLUMNS)[number]>> = {
    STUDENT: ['class', 'section', 'rollNumber'],
  ...
  const downloadTemplate = () => {
    try {
      ...
      const instructions = XLSX.utils.json_to_sheet([
        ...
        { Field: 'email', Required: 'Yes', Values: 'Valid email', Notes: 'Required for import' },
        { Field: 'rfidCode', Required: 'Yes', Values: 'Text', Notes: 'RFID card code' },
        ...
        { Field: 'rollNumber', Required: 'STUDENT only', Values: 'Text', Notes: 'Student roll number' },
  ...
  const getPreviewValue = (row: ImportRow, key: keyof ImportRow) => normalizeCell(row[key]);
  ...
  const validateRow = (row: ImportRow, rowNumber: number, fileLookup: Map<string, File>): PreviewRow => {
    ...
    if (!normalizeCell(row.rfidCode)) {
      errors.push('RFID code is required');
    }
    ...
    if (userType === 'STUDENT') {
      ...
      if (!normalizeCell(row.rollNumber)) errors.push('Roll number is required for STUDENT');
    }
  ...
  const uploadFileToS3 = async (file: Blob, fileName: string) => {
    ...
    return signedUrl.publicUrl || signedUrl.fileUrl || '';
  };
  ...
  const processProfilePhoto = async (photo: string | File | undefined) => {
    ...
    const s3Url = await uploadFileToS3(blob, fileName);
  ...
  const buildPayloadFromRow = async (row: PreviewRow) => {
    const profilePhoto = row.resolvedProfilePhotoFile || row.profilePhoto ? await processProfilePhoto(row.resolvedProfilePhotoFile || row.profilePhoto) : undefined;
