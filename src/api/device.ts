...
export async function getDevicesByPartner(partnerId: string) {
  const res = await fetch(`${BACKEND_BASE_URL}/devices?partnerId=${partnerId}`);
...
export async function getDevicesForPlarform() {
  const res = await fetch(`${BACKEND_BASE_URL}/devices?scope=PLATFORM_ADMIN`);
  if (!res.ok) {
    const data = await res.json().catch(() => ({}));
    throw new Error(data.message || "Failed to fetch devices");
  }
...
export async function getDeviceById(deviceId: string) {
  const res = await fetch(`${BACKEND_BASE_URL}/devices/${deviceId}`);
